...
public class BooleanRetrievalCompressed extends Configured implements Tool {
  ...
  private ArrayListWritable<PairOfInts> fetchPostings(String term) throws IOException {
    ...
    ArrayListWritable<PairOfInts> pair = new ArrayListWritable<PairOfInts>();
    ByteArrayInputStream byteStream = new ByteArrayInputStream(bytes);
    DataInputStream di = new DataInputStream(byteStream);
    int count = WritableUtils.readVInt(di);
    int prevLeft = 0;
    for(int i = 0; i < count; i++) {
	PairOfInts p = new PairOfInts();
	int left = prevLeft + WritableUtils.readVInt(di);
	prevLeft = left;
	int right = WritableUtils.readVInt(di);
	p.set(left,right);
	pair.add(p);
    }
